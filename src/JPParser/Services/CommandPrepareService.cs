...
public class CommandPrepareService
{
    ...
    public List<string> GetCommands()
    {
        ...
        foreach (var command in commandsDictionary)
        {
            ...
            var uniqCommand =
                string.Format(
                    "\"SELECT DISTINCT [X-Forwarded-For] INTO unique_ips.txt"
                    + " FROM {0}"
                    + " WHERE cs-uri-stem IN ('{1}'; '{2}')\" -i:W3C -o:CSV", 
            ...
            var uniqAdditionalCommand = 
                string.Format(
                    ...
                    + "INTO {0}_u.xml"
                    + " FROM unique_ips.txt\" -i:CSV -o:XML",
                    command.Key);
            ...
            var topCommand =
                string.Format(
                    "\"SELECT TOP 20 [X-Forwarded-For] AS clientip, COUNT(*) AS hits"
                    + " INTO {0}.xml"
                    + " FROM {1}"
                    + " WHERE cs-uri-stem IN ('{2}'; '{3}')"
                    + " GROUP BY [X-Forwarded-For] "
                    + "ORDER BY Hits DESC\" -i:W3C -o:XML", 
